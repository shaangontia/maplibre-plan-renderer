...
// OSM proxy
// ---------------------------------------------------------------------------
router.get("/osm/:z/:x/:y.png", (req, res) => {
    const { z, x, y } = req.params;
    proxyTile(`https://tile.openstreetmap.org/${z}/${x}/${y}.png`, "image/png", { "User-Agent": "PlanViewerTileProxy/1.0" }, res);
});
// ---------------------------------------------------------------------------
...
router.get("/satellite/:z/:x/:y", (req, res) => {
    const { z, x, y } = req.params;
    proxyTile(`https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/${z}/${y}/${x}`, "image/jpeg", {}, res);
});
// ---------------------------------------------------------------------------
// Empty glyph PBFs
// ---------------------------------------------------------------------------
router.get("/fonts/:fontstack/:range.pbf", (_req, res) => {
...
router.get("/style.json", (req, res) => {
    const host = req.headers.host || `localhost:${config_1.PORT}`;
    const baseUrl = `http://${host}`;
    const mode = req.query.mode || "normal";
    const planId = req.query.planId || null;
    const allPlans = (0, plans_1.readDb)();
    ...
    const styleName = mode === "canvas" ? "Canvas (Plan Only)" :
...
router.get("/plan-info", (_req, res) => {
    ...
    const result = plans.map((p) => {
        const bounds = (0, plans_1.getBounds)(p);
        const center = (0, plans_1.getCenter)(p);
...
router.get("/health", (_req, res) => {
    res.json({ status: "ok", plans: (0, plans_1.readDb)().length });
});
